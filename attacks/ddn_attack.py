...
class DDN:
    ...
    def __init__(self,
        ...
        self.quantif_iter = quantif_iter
    ...
    def attack(self, model: nn.Module, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        #worst_norm = torch.max(inputs, 255 - inputs).view(batch_size, -1).norm(p=2, dim=1)
        worst_norm = (torch.ones(inputs.shape)*255).view(batch_size, -1).norm(p=2, dim=1).to(self.device)
        orig = inputs.detach().clone()
        # Setup optimizers
        ...
        for i in range(self.steps):
            ...
            #print(is_adv, l2, best_l2, labels, pred_labels)
            is_smaller = l2 < best_l2
            is_both = is_adv * is_smaller
            #print(l2, best_l2, self.init_norm, is_both)
            adv_found[is_both] = 1
            ...
            #if not self.quantif_iter:
            if self.quantize:
                delta.data.round_()
                #print(delta.data.norm())
            # else:
            #     if i%10==0:
            #         adv_q = quantize_per_dist(norm,delta, orig.clone(), model, labels)
            #         #_, adv_q, _, _, _, _, _ = descente_lambda(delta, orig.clone(), model, labels)
            #     #else:
            #     #    adv_q = quantize_per_dist(norm,delta, orig.clone(), model, labels)
            #     #    #_, adv_q, _, _, _, _, _ = descente_lambda(delta, orig.clone(), model, labels)
            #     delta.data = adv_q
            #     adv_norm = torch.norm(adv_q-orig)
                #norm = torch.full((batch_size,), adv_norm, device=self.device, dtype=torch.float)
                #print(torch.norm(adv_q-orig), norm)
            #print(delta.max(), delta.min())
            delta.data.clamp_(0, 255).sub_(inputs)
        # if self.max_norm:
        ...
        #     if self.quantize:
                ...
                # delta.data.round_()
                # else:
                #     _, best_adv, _, _, _, _, _ = descente_lambda(delta, orig.clone(), model, labels)
                #     delta = best_adv

        return inputs + best_delta, ever_found
