...
parser.add_argument('--model', type=str, default='efficientnet-b0', choices=['alexnet','vgg16', 'googlenet','resnet18', 'resnet50','robust', 'efficientnet'], required=False, help="network to attack")
...
input_path = '/nfs/nas4/bbonnet/bbonnet/datasets/images/test/'
...
for i, data_batch in enumerate(orig_loader, 0):
    ...
    #print(init_pred_label, initial_label)
    ...
    for k in range(orig_batch.shape[0]):
        if adv_label[k]==initial_label[k]:
            results_array[cpt1]=1e5
        elif init_pred_label[k]!=initial_label[k]:
            results_array[cpt1]=0
        else:
            disto = (adversarial_image[k]-orig_batch[k]).norm().cpu().numpy()
            results_array[cpt1]= disto
        cpt1+=1
    print(cpt1)
np.save('/nfs/nas4/bbonnet/bbonnet/distortion_curves/{}_{}_{}.npy'.format(model_name, robust, attack_name), results_array)
